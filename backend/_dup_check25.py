"""Batch-25 duplicate/rephrase checker – two independent methods."""
...
NEW_QIDS = set(range(5166, 5186))
NEW_OIDS = set(range(28123, 28243))
...
if m1_q_hits:
    print(f"  ⚠ Question hits: {len(m1_q_hits)}")
    for h in m1_q_hits:
...
if m2_q_hits:
    print(f"  ⚠ Question hits: {len(m2_q_hits)}")
    for h in m2_q_hits:
        print(f"    Q{h[0]} vs Q{h[1]} J={h[2]}  |  {h[3]}  ↔  {h[4]}")
else:
    print("  ✔ No similar questions found")
...
if m2_o_hits:
    print(f"  ⚠ Option hits: {len(m2_o_hits)}")
...
if total_flags == 0:
    print("  ✅ BATCH 25 PASSED BOTH CHECKS — no duplicates or rephrases detected")
else:
