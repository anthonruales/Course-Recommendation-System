"""Batch-27 duplicate/rephrase checker – two independent methods."""
import sys, re
sys.path.insert(0, ".")
...
NEW_QIDS = set(range(5206, 5226))
NEW_OIDS = set(range(28363, 28483))
...
for nq in new_qs:
    ...
    for oq in old_qs:
        j = jaccard(nw, words(oq["question_text"]))
        if j > 0.70:
...
if total_flags == 0:
    print("  BATCH 27 PASSED BOTH CHECKS — no duplicates or rephrases detected")
else:
